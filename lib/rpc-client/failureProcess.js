let logger = require('omelo-logger').getLogger('omelo-rpc', 'failprocess');
...
let failover = function(code, tracer, serverId, msg, opts, cb) {
	...
	if (!servers.length) {
		logger.error('[omelo-rpc] rpc failed with all this type of servers, with serverType: %s', serverType);
