let logger = require('omelo-logger').getLogger('omelo-rpc', 'MailStation');
...
pro.dispatch = function(tracer, serverId, msg, opts, cb) {
  ...
  if (this.state !== STATE_STARTED) {
    ...
    logger.error('[omelo-rpc] client is not running now.');
  ...
  if (!mailbox) {
    ...
    if (!lazyConnect(tracer, this, serverId, this.mailboxFactory, cb)) {
      ...
      logger.error('[omelo-rpc] fail to find remote server:' + serverId);
  ...
  let send = function(tracer, err, serverId, msg, opts) {
    ...
    if (!mailbox) {
      ...
      logger.error('[omelo-rpc] could not find mailbox with id:' + serverId);
    ...
    mailbox.send(tracer, msg, opts, function(tracer_send, send_err, args) {
      ...
      if (send_err) {
        logger.error('[omelo-rpc] fail to send message %s', send_err.stack || send_err.message);
...
pro.connect = function(tracer, serverId, cb) {
  ...
  mailbox.connect(tracer, function(err) {
    if (!!err) {
      ...
      logger.error('[omelo-rpc] mailbox fail to connect to remote server: ' + serverId);
...
let doFilter = function(tracer, err, serverId, msg, opts, filters, index, operate, cb) {
  ...
  if (typeof filter === 'function') {
    filter(serverId, msg, opts, function(target, message, options) {
      index++;
      //compatible for omelo filter next(err) method
...
let lazyConnect = function(tracer, station, serverId, factory, cb) {
  ...
  if (!server) {
    logger.error('[omelo-rpc] unknown server: %s', serverId);
    return false;
  }
  if (!online || online !== 1) {
    logger.error('[omelo-rpc] server is not online: %s', serverId);
    return false;
  }
...
let addToPending = function(tracer, station, serverId, args) {
  ...
  if (pending.length > station.pendingSize) {
    ...
    logger.warn('[omelo-rpc] station pending too much for: %s', serverId);
    return;
  }
  pending.push(args);
};
...
let flushPending = function(tracer, station, serverId, cb) {
  ...
  if (!mailbox) {
    ...
    logger.error('[omelo-rpc] fail to flush pending messages for empty mailbox: ' + serverId);
  }
...
let errorHandler = function(tracer, station, err, serverId, msg, opts, flag, cb) {
  if (!!station.handleError) {
    ...
    logger.error('[omelo-rpc] rpc filter error with serverId: %s, err: %j', serverId, err.stack);
